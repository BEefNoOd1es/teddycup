...
为方便评委和测试人员快速运行本项目，我们提供了 `示例/`（或 `程序跑通/`）文件夹。该文件夹包含了处理好的本地数据源，无需配置爬虫环境或 API Key 即可直接运行回测流水线，复现结果。
