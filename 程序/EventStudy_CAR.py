...
def load_stock_data(filepath):
    df = pd.read_csv(filepath, skiprows=2, encoding='gbk', sep=r'\s+', header=None, on_bad_lines='skip', engine='python')
    if df.shape[1] < 5:
        df = pd.read_csv(filepath, skiprows=3, encoding='gbk', sep=r'\s+', header=None, on_bad_lines='skip', engine='python')
    if df.shape[1] < 5:
        return pd.DataFrame()
    df = df.iloc[:, [0, 1, 4]]
...
def main():
    ...
    for ev in events_list:
        file_path = find_stock_file(ev['code'], export_dir)
        if not file_path:
            print(f"找不到 {ev['code']} 的数据文件")
            continue
